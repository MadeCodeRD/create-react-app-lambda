...
} from "../../../../redux/slices/HappyQ_Company/Company_Booking.js";
import {
...
const EnhancedBookingTableHead = (props) => {
  const {
    onSelectAllClick,
    order,
    orderBy,
...
function FilterBookingDialog(props) {
  const { open, onClose, ticketListFilter } = props;
  const { company, state, date, priority, bookingData } = useSelector(
    (state) => state.happyqCompanyBooking
  );
...
const EnhancedBookingTableToolbar = (props) => {
  // Here was 'let'
...
function BookingTable() {
  ...
  const { bookingData } = useSelector((state) => state.happyqCompanyBooking);
  const { companyProfile } = useSelector((state) => state.happyqCompanyProfile);
  const { branchData } = useSelector((state) => state.happyqCompanyBranch);

  const [rows, setRows] = React.useState(bookingData);
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell align="left">
                        {`${companyProfile.name} #${companyProfile.id}`}
                      </TableCell>
