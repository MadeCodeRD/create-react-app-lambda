...
const filterBranchList = ({ branchData, branch, status, date, address }) => {
...
function FilterBranchDialog(props) {
  ...
  const { branchData, branch, status, date, address } = useSelector(
...
function EnhancedBranchTable() {
  ...
  const dispatch = useDispatch();
  const { branchData, branch, status, date, address, showModal } = useSelector(
    (state) => state.happyqCompanyBranch
  );
...
function Branch() {
  const { showModal } = useSelector((state) => state.happyqCompanyBranch);
  const dispatch = useDispatch();
