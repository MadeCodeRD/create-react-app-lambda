  ...
  if (company.length !== 0) {
    employeeList = employeeList.filter((employeeVal) =>
      company
        ...
        .includes(employeeVal.companyId)
    );
  }
...
function FilterEmployeeDialog(props) {
  ...
  const { employeeData, employee, status, date, role, company, service } =
    useSelector((state) => state.happyqAdminEmployee);
...
function Employee() {
  const { showModal } = useSelector((state) => state.happyqAdminEmployee);
  const dispatch = useDispatch();
